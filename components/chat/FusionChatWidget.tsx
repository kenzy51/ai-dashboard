...
export default function FusionChatWidget() {
  ...
  const [messages, setMessages] = useState([
    { role: "assistant", content: "System Online. I'm Sarah—your TRT logistics co-pilot. How can I accelerate your freight today?" }
  ]);
  ...
  return (
    <div className="fusion-ai-app-container relative text-left antialiased">
      <div className={`fixed z-[9999] transition-all duration-500 cubic-bezier(0.16, 1, 0.3, 1) flex flex-col
        ${isOpen 
        ...
        {isOpen ? (
          <>
            ...
            <div className="pt-[env(safe-area-inset-top,24px)] md:pt-10 pb-6 px-8 flex justify-between items-center shrink-0 border-b border-white/5 bg-gradient-to-b from-zinc-900/40 to-transparent">
              ...
              <button onClick={() => setIsOpen(false)} className="close-btn w-10 h-10 flex items-center justify-center bg-white/5 rounded-full text-zinc-400 hover:text-white hover:bg-white/10 transition-all">
                <X size={20} />
              </button>
            </div>
            ...
            <div ref={scrollRef} className="flex-1 overflow-y-auto px-8 py-6 space-y-8 scrollbar-hide overscroll-contain">
              ...
              {messages.length > 2 && !isTyping && (
                <div className="pt-6 flex justify-center animate-in zoom-in duration-700">
                  <button onClick={() => window.location.href="tel:+19297022797"} className="call-btn group relative flex items-center gap-4 bg-white text-black px-10 py-5 rounded-full font-black text-xs uppercase tracking-[0.2em] hover:scale-105 active:scale-95 shadow-[0_20px_40px_rgba(255,255,255,0.1)] transition-all">
                    <PhoneCall size={16} fill="black" className="group-hover:rotate-12 transition-transform" /> 
                    Secure Line
                  </button>
                </div>
              )}
              ...
              {isTyping && (
                <div className="flex items-center gap-3 pl-2 opacity-50">
                  ...
                  <span className="text-[10px] text-[#d4ff33] font-black tracking-widest uppercase">Analyzing...</span>
                </div>
              )}
            </div>
            ...
            <div className="p-8 bg-gradient-to-t from-zinc-950 to-transparent shrink-0">
              <div className="relative flex items-center group">
                ...
                />
                <button onClick={sendMessage} className="send-btn absolute right-3 w-14 h-14 bg-[#d4ff33] rounded-2xl text-black flex items-center justify-center hover:shadow-[0_0_25px_rgba(212,255,51,0.6)] group-active:scale-90 transition-all">
                  <Send size={24} strokeWidth={3} />
                </button>
              </div>
              <div className="flex justify-between items-center mt-6 px-2">
                <p className="text-[8px] text-zinc-700 font-black tracking-[0.5em] uppercase">Fusion AI v3.0</p>
                <div className="flex gap-1">
          ...
          /* PRESTIGE TRIGGER */
          <button onClick={() => setIsOpen(true)} className="launcher-btn group relative bg-[#d4ff33] w-20 h-20 rounded-[32px] flex items-center justify-center shadow-[0_25px_50px_-12px_rgba(212,255,51,0.4)] hover:scale-110 active:scale-90 transition-all duration-500">
