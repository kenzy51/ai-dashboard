...
export default function ChatDetailPage() {
  ...
  if (isLoading) return <div className="p-20 text-[#d4ff33] animate-pulse font-mono uppercase tracking-widest text-center">Decrypting Transcript...</div>;
  if (!chat) return <div className="p-20 text-zinc-500 text-center">Transcript not found.</div>;
  ...
  return (
    <div className="max-w-6xl mx-auto p-8 space-y-8 animate-in fade-in slide-in-from-bottom-4 duration-700">
      <button onClick={() => router.push("/chats")} className="flex items-center gap-2 text-zinc-500 hover:text-[#d4ff33] transition-colors group uppercase text-[10px] font-bold tracking-widest">
      ...
      <div className="flex justify-between items-end border-b border-zinc-800/50 pb-8">
        <div className="space-y-1">
          <span className="text-[#d4ff33] text-[10px] font-bold uppercase tracking-[0.2em]">Logistics Intelligence / {chat.procedure}</span>
          <h1 className="text-5xl font-bold text-zinc-100 tracking-tighter">{chat.patientPhone}</h1>
        </div>
            ...
            {messages.map((msg, i) => (
              <div key={i} className={`flex flex-col ${msg.role === "user" ? "items-end" : "items-start"}`}>
                ...
                <div className={`max-w-[85%] px-5 py-3 rounded-[24px] text-sm leading-relaxed ${
                    msg.role === "user" ? "bg-zinc-800 text-zinc-200 rounded-tr-none" : "bg-[#d4ff33] text-black font-medium rounded-tl-none shadow-lg shadow-[#d4ff33]/5"
                  }`}>
                  {msg.content}
                </div>
              </div>
          ...
          <div className="bg-zinc-900/30 backdrop-blur-md border border-zinc-800/50 rounded-[40px] p-8 h-fit space-y-8">
            <div>
              <h3 className="text-zinc-100 font-bold text-[10px] tracking-widest uppercase flex items-center gap-2 mb-4">
                <FileText className="h-4 w-4 text-[#d4ff33]" /> AI Intelligence Summary
              </h3>
            ...
            <div className="space-y-4 border-t border-zinc-800 pt-6">
               <h3 className="text-zinc-100 font-bold text-[10px] tracking-widest uppercase mb-4 tracking-widest font-bold">Audio Recording</h3>
               <audio className="w-full h-10 filter invert grayscale opacity-80 hover:opacity-100 transition-opacity" controls src={`${process.env.NEXT_PUBLIC_SERVER_URL}/calls/stream-recording?url=${encodeURIComponent(chat.recordingUrl || '')}`} />
            </div>
